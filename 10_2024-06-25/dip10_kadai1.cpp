...
{
    // 画像のファイルパスを指定
    std::string imagePath = "./src/nybuildings.jpg";  // または "./src/lenna_g.jpg"

    //①原画像のグレースケール画像を"sourceImg"に格納
    cv::Mat sourceImg = cv::imread(imagePath, cv::IMREAD_GRAYSCALE);
    ...
    // ローパスフィルタの適用
    double lowRadius = 30.0;  // 適用する半径
    applyLowPassFilter(ftMatrix, lowRadius);
    ...
    //画像の保存
    cv::imwrite("./dst/dip10_kadai1.jpg", resultImg);
